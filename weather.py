...
def help_handler(update, context:CallbackContext):
    update.message.reply_text("Write /temp city_name, city_name is a city where you want to know daily weather forecast."
                              "\nFor example, \n\t\t/temp Berlin\n\t\t/temp Kiev. \n/geo \n"
                              "To get weather forecast by your location coordinates.\n"
...
def temperature_handler_5(update,context):
    ...
    loc ="".join(context.args)
    result=get_weather_5(loc)
    update.message.reply_text("".join(result))

def main():
    updater = Updater(WTOKEN, use_context=True)
