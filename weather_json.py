...
def get_weather(loc):
    ...
    msg='{}. Daily weather forecast in {}, {}: {}°C, feels like {}°C, {}.'.format(day, city, country, temp, feels_like,"".join(main))
    return msg
...
def geo_weather_5(lon,lat):
    url = 'http://api.openweathermap.org/data/2.5/forecast?lat={}&lon={}&units=metric&cnt=5&appid={}'.format(lat, lon, api_key)
    data = requests.get(url)
    response = json.loads(data.content.decode('utf8'))
    # json request has {keys:values} pairs, data separation
    city = response['city']['name']
